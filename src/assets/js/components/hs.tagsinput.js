    ...
    initInputs: function () {
      ...
      return this.collection.each(function (i, el) {
        ...
        if (listManual) {
          ...
          $this.on('add', function(e, tagName){
            if(tagify.suggestedListItems.length === 1) {
              $(tagify.DOM.dropdown).empty().fadeOut(0);
            }
          });

          $this.on('remove', function (e, tagname) {
            if(tagify.suggestedListItems.length === 0) {
              $(tagify.DOM.dropdown).fadeIn(0);
            }
          })
        }
